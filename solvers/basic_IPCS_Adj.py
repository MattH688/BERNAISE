...
def setup(tstep, test_functions, trial_functions, w_, w_1, bcs, permittivity,
    ...
    if enable_NS:
        solvers["NSu"] = setup_NSu(
            ...
            enable_PF, enable_EC, use_iterative_solvers)
        solvers["NSp"] = setup_NSp(p, q, dirichlet_bcs["NSp"], u_, p_, p_1, rho_, dt,use_iterative_solvers)
...
def setup_NSu(u, v, u_, p_, bcs_NSu,
    ...
    solver["a1"] = a1
    A1 = df.assemble(a1)
    [bc.apply(A1) for bc in bcs_NSu]
    solver["A1"] = A1
    solver["L1"] = L1
    solver["Fu"] = F_predict
    solver["Fu_corr"] = F_correct
    solver["a3"] = a3
    solver["A3"] = df.assemble(a3)
    solver["L3"] = L3
    solver["bcs"] = bcs_NSu
    return solver


def setup_NSp(p, q, bcs_NSp, u_, p_, p_1, rho_, dt,use_iterative_solvers):
    F_correct = (
...
def solve(tstep, w_, w_1, w_tmp, solvers,
          enable_PF, enable_EC, enable_NS,freeze_NSPF, use_iterative_solvers,
          **namespace):
    """ Solve equations. """
    ...
    #         timer_inner.stop()
    for subproblem, enable in zip(["PF", "EC"],
    ...
    if enable_NS:
        ...
        # # Needs to be imported from sources otherwise always rebuilding
        ...
        # timer.start()
        # [bc.apply(solvers["NSu"]["A1"]) for bc in solvers["NSu"]["bcs"]]
        # [bc.apply(solvers["NSu"]["A2"]) for bc in solvers["NSp"]["bcs"]]
        # timer.stop()
        ...
        while du > tol and i_iter < max_num_iterations:
            ...
            # timer_0 = df.Timer("NS: Tentative velocity")
            # timer_0.start()
            ...
            timer_0.start()
            mpi_barrier()
            df.solve(solvers["NSu"]["A1"], w_["NSu"].vector(), b1,'bicgstab', 'hypre_amg')
            timer_0.stop()
            # df.solve(A1, w_["NSu"].vector(), b1,'bicgstab', 'hypre_amg')
            
            # problem = df.LinearVariationalProblem(A1, w_["NSu"].vector(), b1)
            ...
            # timer_1 = df.Timer("NS: Pressure correction")
            # timer_1.start()
            ...
            timer_1.start()
            mpi_barrier()
            df.solve(solvers["NSp"]["A2"], w_["NSp"].vector(), b2,'bicgstab', 'hypre_amg')
            timer_1.stop()
            # df.solve(A2, w_["NSp"].vector(), b2,'bicgstab', 'hypre_amg')
            
            # problem = df.LinearVariationalProblem(A2, w_["NSp"].vector(), b2)
            # solver = df.LinearVariationalSolver(problem)
        ...
        # timer_2 = df.Timer("NS: Velocity correction")
        # timer_2.start()
        ...
        timer_2.start()
        mpi_barrier()
        df.solve(solvers["NSu"]["A3"], w_["NSu"].vector(), b3,'cg', 'sor')
        # df.solve(A3, w_["NSu"].vector(), b3,'cg', 'sor')
        
        # problem = df.LinearVariationalProblem(A3, w_["NSu"].vector(), b3)
        # solver = df.LinearVariationalSolver(problem)
...
def solve_initial_pressure(w_NSp, p, q, u, v, bcs_NSp,
    ...
    problem = df.LinearVariationalProblem(df.lhs(F_grad_p), df.rhs(F_grad_p), grad_p_out)
    solver = df.LinearVariationalSolver(problem)
