...
def setup(mesh, test_functions, trial_functions, w_, w_1,
          dx, ds, normal,
          ...
          grav_const,
          grav_dir, pf_mobility_coeff, pf_mobility,
          q_rhs,
...
def setup_NSPFEC(w_NSPFEC, w_1NSPFEC,
    ...
    if enable_NS:
        ...
        ## Oasis Code
        # https://github.com/mikaem/Oasis/blob/master/oasis/solvers/NSCoupled/default.py
        # F_nonlinear = inner(dot(grad(u_), u_), v) * dx()
        # F_linear = (nu * inner(grad(u_), grad(v)) * dx() - inner(p_, div(v)) * dx()
        #             - inner(q, div(u_)) * dx())

        F_NS_nonlinear = df.inner(df.dot(df.grad(u_), u_), v) * dx()
        F_NS_linear = (nu_ * df.inner(df.grad(u_), df.grad(v)) * dx()
                    - df.inner(p_, df.div(v)) * dx()
                    - df.inner(q, df.div(u_)) * dx())

        ## Partial Mix of Oasis and BERNAISE

        # F_NS_nonlinear = (df.inner(df.nabla_grad(u_), df.outer(mom_, v)) * dx)
        
        # F_NS_linear = (2*nu_*df.inner(df.sym(df.nabla_grad(u_)), 
        #                                 df.sym(df.nabla_grad(v))) * dx
        #         - p_ * df.div(v) * dx
        #         - df.div(u_) * q * dx
        #         - df.dot(rho_ * grav, v) * dx)

        F_NS = F_NS_linear + F_NS_nonlinear
        
        # J_linear = df.derivative(F_NS_linear, w_1NSPFEC, w_NSPFEC)
        # J_nonlinear = df.derivative(F_NS_nonlinear, w_1NSPFEC, w_NSPFEC)
        #J_linear = derivative(F_linear, up_, up)
        #J_nonlinear = derivative(F_nonlinear, up_, up)


        # A_pre = df.assemble(J_linear)
        # # print(A_pre)
        # A = df.GenericMatrix(A_pre)

        # A = df.assemble(J_nonlinear, tensor=A_pre)
        ...
        # # From NS_solve (Oasis)
        # up_1.vector().zero()
        # up_sol.solve(A, up_1.vector(), w_NSPFEC) # up
        # up_.vector().axpy(-omega, up_1.vector())
        # b["up"] = assemble(F, tensor=w_NSPFEC)
        # for bc in bcs["up"]:
        ...
        # if use_pressure_stabilization:
            # # Residual
            # r = u_ - u_1 + df.dot(w_NSPFEC, df.grad(u_)) - nu_*df.div(df.grad(u_))
            # #r = u - u_n + dot(w, grad(u_mid)) - eps*div(grad(u_mid))
            # # Add SUPG stabilisation terms
            # vnorm = df.sqrt(df.dot(w_NSPFEC, w_NSPFEC))
            # h = df.CellDiameter(mesh)
            # delta = h/(2.0*vnorm)
            # F_NS += delta*df.dot(w_NSPFEC, df.grad(p_1))*r*dx
    ...
    # problem_NSPFEC = df.NonlinearVariationalProblem(A, up_1.vector(), w_NSPFEC)
    # problem_NSPFEC = df.NonlinearVariationalProblem(F, w_NSPFEC, dirichlet_bcs_NSPFEC)
    ...
    if use_iterative_solvers:
        # solver_NSPFEC.parameters["newton_solver"]["relative_tolerance"] = 1e-6
        solver_NSPFEC.parameters['newton_solver']['linear_solver'] = 'gmres'
        solver_NSPFEC.parameters['newton_solver']['preconditioner'] = 'ilu'#"hypre_euclid"
